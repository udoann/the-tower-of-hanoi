<<<<<<< HEAD
"""Four-in-a-Row, by Nwankwo Emeka usmekuzee93@gmail.com
...
def main():
    ...
    print(
        """Four-in-a-Row, by Nnaemeka N usmekuzee93@gmail.com
...
def displayBoard(board):
    """Diplay the board check on page 262 and complete."""
    ...
    # spaces) going left to right, tp to bottom:
    tileChars = []
    for rowIndex in range(BOARD_HEIGHT):
        for columnIndex in range(BOARD_WIDTH):
...
def getPlayerMove(playerTile, board):
    ...
    while True:  # Keep asking player until they enter a valid move.
        ...
        if response not in COLUMN_LABELS:
            print(f"Enter a number from ! to {BOARD_WIDTH}.")
            continue  # Ask player again for their move.

        columnIndex = int(response) - 1  # -1 for )-based column indexex.
...
def isFull(board):
    """REturns True if the board has no empty spaces, other wise 
    returns False."""
    for rowIndex in range(BOARD_HEIGHT):
        for columnIndex in range(BOARD_WIDTH):
            if board[(columnIndex, rowIndex)] == EMPTY_SPACE:
...
def isWinner(playerTile, board):
    ...
    for columnIndex in range(BOARD_WIDTH):
        for rowIndex in range(BOARD_HEIGHT - 3):
            # heck for four in a row going down:
            tile1 =board[(columnIndex, rowIndex)]
...
# if this program was run (instead of imported), run the game:
if __name__ =="__main__":
>>>>>>> 4fa140f6c5cff6e88310d486454c23772b75c9c5
    main()